...
to set_index
  set total-exits 0
  set possible_hundredexit_profit (-1 * ln 100) / (constant_b * ln constant_a)
...
to random_move_and_select_pub
  ask consumers [
    set target-pub nobody
    let tries 0

    while [target-pub = nobody and tries < 100] [
      ;; 内层循环：找一个合法方向
      let found-legal? false
      let attempts 0

      while [not found-legal? and attempts < 8] [ ;; 最多转8次，每次45度（或90度）
        let next-patch patch-ahead 1
        ifelse [insector?] of next-patch [
          fd 1
          set found-legal? true
        ] [
          rt 45   ;; 或 rt 90，你可以自己调整
      ...
      let pubs-here pubs-on patch-here
      if any? pubs-here [
        set target-pub one-of pubs-here
      ]
    ]
  ]
end
...
to calculate_probabilityexit_index
  ask pubs [
    ...
    let exit_probability_tempor constant_a ^ (-1 * constant_b * k) - 1
    if exit_probability_tempor <= 1 [set exit_probability_tempor 0]
...
to check_if_annual_profit_is_negative
  ifelse any? pubs with [annual_profit < 0] [
    show "There is at least one pub with annual profit less than 0."
  ]
  [
    show "No pub has annual profit less than 0."
  ]
end

to check_min_annual_profit
  let min-profit min [annual_profit] of pubs
  let min-pub one-of pubs with [annual_profit = min-profit]
  show (word "The pub with the minimum annual profit (" min-profit ") is: " [name] of min-pub)
end

to calculate_average_exit_probability
  let avg-exit-probability mean [exit_probability] of pubs
  show (word "The average exit probability for all pubs is: " avg-exit-probability)
end

to calculate_average_profit
  let avg-profit mean [annual_profit] of pubs
  show (word "The average annual profit for all pubs is: " avg-profit)
end

to calculate_min_profit
  let min-profit min [annual_profit] of pubs
  let min-pub one-of pubs with [annual_profit = min-profit]
  show (word "The pub with the minimum annual profit (" min-profit ") is: " [name] of min-pub)
end

to check_if_annual_profit_below_possible_exit_profit
  ifelse any? pubs with [annual_profit < possible_hundredexit_profit] [
    show "There is at least one pub with annual profit below possible_hundredexit_profit."
  ]
  [
    show "No pub has annual profit below possible_hundredexit_profit."
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
<experiments>
  ...
  <experiment name="experiment (copy)" repetitions="8" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
