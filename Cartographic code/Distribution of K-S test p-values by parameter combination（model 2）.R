...
EG <- read.csv("Gravity Model-Based (Single-Spending, Model 2).csv", skip = 6)
...
for (i in seq_along(run_ids)) {
  ...
  run_data <- subset(EG, X.run.number. == run_i & X.step. >= 3 & X.step. <= 13)$exit.rate
...
ks_results_params %>%
  ...
  coord_flip() +
  labs(
    title = "p_value distribution (Model 2)",
