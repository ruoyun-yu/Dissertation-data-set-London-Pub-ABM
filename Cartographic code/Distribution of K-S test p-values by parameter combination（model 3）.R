...
EG <- read.csv("Intervening Opportunities Model-Based (Single-Spending, Model 3).csv", skip = 6)
...
# Step 5: Loop through each run and perform the Kolmogorov-Smirnov test
for (i in seq_along(run_ids)) {
  run_i <- run_ids[i]
  
  # Filter rows: only include steps that are multiples of 12, between 120 and 250
  run_data <- subset(EG, X.run.number. == run_i & X.step. %% 12 == 0 & X.step. >= 120 & X.step. <= 240)$exit.rate
...
ks_summary <- ks_summary %>%
  mutate(p_group = ifelse(mean_p > 0.05, 1, 0)) %>%
...
ks_results_params %>%
  ...
  geom_hline(yintercept = 0.05, linetype = "dashed", color = "skyblue", size = 0.6) +
  scale_y_continuous(
  ...
  coord_flip() +
  labs(
    title = "p_value distribution (Model 3)",
